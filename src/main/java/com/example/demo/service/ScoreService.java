...
public class ScoreService {
    ...
    public ScoreResult getScoreListByFilter(String point,String page,String attendance,String barrage,String submission,String announcement, String average, String courseName, String stuID, Map<Object,Object> setting, int pagenum, int pagesize) throws SQLException, ClassNotFoundException {
        ...
        if(!point.isEmpty()) {
            // 遍历list  取反删除来实现
            scoreList.removeIf(score -> Integer.parseInt(score.get(colName[3])) >= Integer.parseInt(point));
        }
        if(!page.isEmpty()) {
            scoreList.removeIf(score -> Integer.parseInt(score.get(colName[4])) >= Integer.parseInt(page));
        }
        if(!attendance.isEmpty()) {  // 缺勤次数多于x 即 签到次数小于总-x 再取反
            int totalAttendance = sqlUtil.getTotalAttendance(courseName);
            scoreList.removeIf(score -> Integer.parseInt(score.get(colName[5])) >= (totalAttendance - Integer.parseInt(attendance)));
        }
        if(!barrage.isEmpty()) {
            scoreList.removeIf(score -> Integer.parseInt(score.get(colName[7])) >= Integer.parseInt(barrage));
        }
        if(!submission.isEmpty()) {
            scoreList.removeIf(score -> Integer.parseInt(score.get(colName[8])) >= Integer.parseInt(submission));
        }
        if(!announcement.isEmpty()) {
            scoreList.removeIf(score -> Integer.parseInt(score.get(colName[9])) >= Integer.parseInt(announcement));
        }
        if(!average.isEmpty()) {
